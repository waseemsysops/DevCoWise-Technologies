...
export default function SolutionsView({ isDark, setCurrentTab }: { isDark: boolean, setCurrentTab: (tab: string) => void }) {
  const [selectedInd, setSelectedInd] = useState<string>('healthcare');
  ...
  const faqs = [
    ...
    {
      q: "Can we integrate open-core ERPNext with our current legacy mainframe?",
      a: "Absolutely. We construct customized GraphQL or REST API gateways that fetch ledger logs from legacy DB2 or Oracle mainframes, syncing records to modern MariaDB/PostgreSQL databases with zero downtime."
    },
    {
  ...
  return (
    <div className="space-y-20 pb-20 pt-10">
      ...
      <section className="text-center max-w-3xl mx-auto space-y-6 px-6">
        ...
        <p className={`text-sm leading-relaxed ${isDark ? 'text-gray-355' : 'text-gray-600'}`}>
              ...
              <button
                ...
                className={`px-6 py-3 rounded-xl border flex items-center space-x-2.5 transition-all text-xs font-semibold cursor-pointer ${
                  selectedInd === ind.id
                    ...
                    : isDark
                      ? 'bg-card-dark border-gray-800 text-gray-400 hover:text-white hover:bg-gray-850'
                      : 'bg-white border-gray-150 text-gray-600 hover:bg-gray-50 hover:text-gray-950 hover:shadow-sm'
                }`}
              >
      ...
      <section className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          {/* Success Story Box */}
          <div className={`p-6 rounded-2xl border ${isDark ? 'bg-gray-800/20 border-gray-800' : 'bg-gray-50 border-gray-150 shadow-sm'}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {current.techStack.map((tech, idx) => (
                <span key={idx} className={`px-3 py-1.5 rounded-xl text-xs font-medium font-mono ${
                  isDark ? 'bg-gray-800/50 text-gray-300' : 'bg-white border border-gray-150 text-gray-600'
                }`}>
                  {tech}
                </span>
              ))}
